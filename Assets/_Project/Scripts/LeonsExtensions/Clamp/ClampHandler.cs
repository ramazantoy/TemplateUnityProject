...
namespace _Project.Scripts.LeonsExtensions.Clamp
{
    ...
    public static class ClampHandler
    {
        ...
        public static Vector3 ClampVector(this Vector3 targetVector, VectorClamp vectorClamp)
        {
            float x = Mathf.Clamp(targetVector.x, vectorClamp.X.Min, vectorClamp.X.Max);
            float y = Mathf.Clamp(targetVector.y, vectorClamp.Y.Min, vectorClamp.Y.Max);
            float z = Mathf.Clamp(targetVector.z, vectorClamp.Z.Min, vectorClamp.Z.Max);
